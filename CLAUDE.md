...
`filepath` (required), `-n/--num-lineups`, `-u/--min-uniques`, `-e/--export`, `-l/--lock`, `-x/--exclude`, `-s/--stack N` (QB + N WR/TE same team), `-srb/--stack-rb`, `-te/--max-te`, `-ndo/--no-dst-opp`, `-ms/--max-salary` (Showdown only), `-c/--ceiling` and `-pj/--projceiling` (multi-lineup scripts only; mutually exclusive).
