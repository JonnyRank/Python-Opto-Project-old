...
- Display slot assignment is post-hoc, not part of the model: `_assign_flex_positions()` fills RB/WR/TE slots by descending salary and drops the leftover FLEX-eligible player into FLEX. In the classic script, `main()` first maps a `Kickoff` column onto the players by ID from `load_dk_kickoffs()` (newest `DKEntries*.csv` in `DOWNLOADS_DIR`, `Game Info`); when present, the surplus position holds back its latest kickoff for FLEX instead of its cheapest (tie → cheapest). No file, or no match, reduces to the salary rule; the printed table only shows the `Kickoff` column when some player matched, and exports drop it via `EXPORT_COLUMNS`.
