...
`filepath` (required), `-n/--num-lineups`, `-u/--min-uniques`, `-e/--export`, `-l/--lock`, `-x/--exclude`, `-s/--stack N` (QB + N WR/TE same team), `-srb/--stack-rb`, `-te/--max-te`, `-ndo/--no-dst-opp`, `-ms/--max-salary` (Showdown only).
...
Classic NFL: `ID`, `Player`, `Position`, `Team`, `Opp`, `Salary`, `Proj`→`Projection`, `Own`→`Ownership`, `Ceiling`. `Opp` may be `"@BUF"` or `"BUF"`. Salary strings like `"$6,000 "` and ownership like `"11.70%"` are stripped to numerics; rows missing critical columns are dropped.
