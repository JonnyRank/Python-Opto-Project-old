...
Example:
    python NFL-Single-Opto.py "C:\\path\\to\\projections.csv" -n 5 -u 2
...
def main() -> None:
    ...
    try:
        ...
        for i in range(args.num_lineups):
            ...
            # Add diversity constraints for all previously generated lineups
            for prev_lineup_indices in generated_lineups_indices:
                prob += (
                    pulp.lpSum(player_vars[p_idx] for p_idx in prev_lineup_indices)
                    <= max_players_can_share,
                    f"Diversity_from_lineup_{len(generated_lineups_indices)}",
                )
            ...
            print(f"Total Projection: {projection:.2f}")
